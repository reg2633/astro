...
kst_time = now + timedelta(hours=9)  # KST는 UTC+9
st.sidebar.write(f"현재 시간: {now.strftime('%Y-%m-%d %H:%M:%S')} UTC (KST: {kst_time.strftime('%H:%M:%S')})")
    ...
    try:
        with fits.open(uploaded_file) as hdul:
            ...
            if image_hdu is None:
                ...
                # --- 천체 종류 추출 또는 사용자 입력 ---
                object_type = header.get('OBJECT', '알 수 없음')
                st.sidebar.header("🌟 천체 정보")
                object_type_input = st.sidebar.selectbox(
                    "천체 종류",
                    options=["알 수 없음", "별", "은하", "성운", "행성", "기타"],
                    index=0 if object_type == '알 수 없음' else ["별", "은하", "성운", "행성", "기타"].index(object_type) if object_type in ["별", "은하", "성운", "행성", "기타"] else 0,
                    help="FITS 헤더의 OBJECT 필드에서 읽거나 선택하세요."
                )
                if object_type_input == "기타":
                    object_type_input = st.sidebar.text_input("천체 종류를 입력하세요", value=object_type if object_type != '알 수 없음' else "")
                if object_type != '알 수 없음':
                    st.sidebar.info(f"FITS 헤더에서 천체 종류: {object_type}")
                ...
                with col1:
                    ...
                    st.text(f"관측 대상: {object_type_input}")
                    st.text(f"거리: {default_distance:.2f} 파섹 ({distance_source})")
                ...
                with col2:
                    ...
                    if data.max() == data.min():
                        ...
                        st.warning("이미지 데이터의 최대/최소값이 동일하여 표시할 수 없습니다.")
                    else:
                ...
                if 'RA' in header and 'DEC' in header:
                    try:
                        ...
                        distance = st.sidebar.number_input(
                            ...
                            step=10.0,
                            key="distance_input_unique",
                            help=f"FITS 헤더에서 읽은 거리: {header_distance if header_distance is not None else '없음'}"
                        )
                        ...
                        if distance <= 0:
                            ...
                            ax.scatter([0], [0], [0], color='yellow', s=100, label='태양')
                            ax.scatter([x], [y], [z], color='blue', s=50, label=f'{object_type_input}')
                            ax.set_xlabel('X (parsec)')
                            ax.set_ylabel('Y (parsec)')
                            ax.set_zlabel('Z (parsec)')
                            ax.set_title(f'{object_type_input}의 3D 위치')
                            ax.legend()
                            ax.grid(True)
                            st.pyplot(fig)
                ...
                if 'RA' in header and 'DEC' in header:
                    try:
                        target_coord = SkyCoord(
                            ra=header['RA'], dec=header['DEC'], distance=distance if distance > 0 else None,
                            unit=('hourangle', 'deg', 'parsec' if distance > 0 else None)
                        )
